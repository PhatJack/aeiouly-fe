...
export const isValidSelection = (
  ...
  const isValidNode =
    isNodeSelection(selection) && !excludeNodeTypes.includes(selection.node.type.name);
...
export const isValidTextSelection = (editor: Editor | null): boolean => {
  ...
  const isEmptyTextBlock =
    isTextSelection(selection) && !doc.textBetween(selection.from, selection.to).length;
...
export function getClosestDOM(
  ...
  let node: DOMNode =
    view.nodeDOM(state.selection.from) || view.domAtPos(state.selection.from).node;
 ...
 */
export function loadInitialContent(editor: Editor | null, content: Content): boolean {
  if (!editor) return false;
...
export function getEditorContent(editor: Editor | null, format: 'json'): Exclude<Content, string>;

export function getEditorContent(editor: Editor | null, format?: 'html' | 'json'): Content;
export function getEditorContent(editor: Editor | null, format: 'html' | 'json' = 'html'): Content {
  if (!editor) return null;
