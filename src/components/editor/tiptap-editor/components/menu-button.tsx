...
interface MenuButtonProps extends Omit<ButtonProps, 'ref' | 'type' | 'iconOnly'> {
  icon?: IconName;
...
export const MenuButton = React.forwardRef<HTMLButtonElement, MenuButtonProps>(
  (
    ...
    const hasArrowIcon = !hideArrow && (type === 'dropdown' || type === 'popover');
    ...
    const tooltipContent = useMemo(() => {
      ...
      const shortcut = shortcuts ? `${shortcuts.map(getShortcutKey).join(' + ')})` : '';
    ...
    const renderIcon = useMemo(
      () =>
        icon ? <DynamicIcon name={icon as any} className="rte-button-icon" size={16} /> : null,
      [icon]
    );
