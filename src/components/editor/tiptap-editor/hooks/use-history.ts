...
// Utility functions
export function canUndoRedo(editor: Editor | null, action: UndoRedoAction): boolean {
  if (!editor || !editor.isEditable) return false;

  return editor.can()[action]();
}

export function executeUndoRedo(editor: Editor | null, action: UndoRedoAction): boolean {
  if (!editor || !editor.isEditable) return false;
