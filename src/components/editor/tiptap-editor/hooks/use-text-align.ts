...
// Utility functions
export function canSetTextAlign(editor: Editor | null, alignment: TextAlignType) {
  if (!editor || !editor.isEditable) return false;
  if (editor.isActive('table')) return false;
  return editor.can().setTextAlign(alignment);
}

export function isTextAlignActive(editor: Editor | null, alignment: TextAlignType) {
