...
export function canExecuteTableAction(editor: Editor | null, action: TableAction): boolean {
...
export function insertTable(editor: Editor | null, options: TableInsertOptions): boolean {
  if (!editor || !editor.isEditable) return false;
...
export function executeTableAction(editor: Editor | null, action: TableAction): boolean {
  if (!editor || !editor.isEditable) return false;
...
export function useTable() {
  ...
  const addRowBefore = useCallback(() => executeTableAction(editor, 'addRowBefore'), [editor]);

  const addRowAfter = useCallback(() => executeTableAction(editor, 'addRowAfter'), [editor]);
  ...
  const addColumnAfter = useCallback(() => executeTableAction(editor, 'addColumnAfter'), [editor]);

  const deleteRow = useCallback(() => executeTableAction(editor, 'deleteRow'), [editor]);

  const deleteColumn = useCallback(() => executeTableAction(editor, 'deleteColumn'), [editor]);
  ...
  const mergeCells = useCallback(() => executeTableAction(editor, 'mergeCells'), [editor]);

  const splitCell = useCallback(() => executeTableAction(editor, 'splitCell'), [editor]);

  const deleteTable = useCallback(() => executeTableAction(editor, 'deleteTable'), [editor]);
