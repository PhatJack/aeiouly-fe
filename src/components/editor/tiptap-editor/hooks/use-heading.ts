...
// Utility functions
export function canToggleHeading(editor: Editor | null, level?: HeadingLevel): boolean {
  if (!editor || !editor.isEditable) return false;
  return editor.can().setNode('heading', { level });
}

export function isHeadingActive(editor: Editor | null, level?: HeadingLevel): boolean {
  if (!editor) return false;
  return level ? editor.isActive('heading', { level }) : editor.isActive('paragraph');
}
...
export function toggleHeading(editor: Editor | null, level?: HeadingLevel): boolean {
  if (!editor) return false;
