...
// Utility functions
export function canSetTextStyle(editor: Editor | null, property: TextStyleProperty): boolean {
  ...
  return editor.can().setMark('textStyle', { [property]: testValues[property] });
}
...
export function setTextStyle(editor: Editor | null, attributes: TextStyleAttributes): boolean {
  if (!editor || !editor.isEditable) return false;
  return editor.chain().focus().setMark('textStyle', attributes).run();
}

export function unsetTextStyle(editor: Editor | null, property: TextStyleProperty): boolean {
  if (!editor || !editor.isEditable) return false;
